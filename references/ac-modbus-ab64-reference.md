...
AB64 เป็นตัวแปลโปรโตคอล ไม่ใช่ Modbus-native device — ฝั่งหนึ่งคุยด้วย **AB Bus** ที่เป็น proprietary ของแอร์ อีกฝั่งคุยด้วย Modbus RTU และ**รับไฟจากฝั่ง AB Bus (จากแอร์) ไม่ใช่จากฝั่ง Modbus** unit-id address space รองรับสูงสุด **64 กล่อง** ต่อ Modbus network เดียวกัน — ตัวเลขนี้คือ**การนับ address ที่มีอยู่ (1–64)** ไม่ใช่บรรทัดที่คู่มือระบุจำนวนอุปกรณ์สูงสุดไว้ตรง ๆ ดูหมายเหตุ provenance ในบล็อกที่สองด้านล่าง
...
> **อนุมาน ไม่ใช่ยืนยันจากคู่มือ (แก้ 2026-08-05, review `unitstep-review.md` M-3):** "สูงสุด 64 กล่อง**ต่อ network**" ไม่ใช่ประโยคที่คู่มือระบุไว้ตรง ๆ — เป็นการ**นับจำนวน address ที่มีอยู่ในตาราง SW1/SW2 (1–64 = 64 ค่า)** แล้วสมมติว่า "จำนวน address ที่มี" เท่ากับ "จำนวนกล่องที่ใส่ได้จริง" ซึ่งไม่จำเป็นต้องจริง คู่มืออาจมีข้อจำกัดทางไฟฟ้า/แบนด์วิดท์ของ RS-485 แยกต่างหากที่ยังไม่ได้ตรวจ (RS-485 มาตรฐานทั่วไปรับ ~32 unit load ต่อ segment ถ้าไม่มี repeater) และตาราง bus-math ในตัวเลือก Poll interval ของ `README.md` เองก็แสดงว่าบัสเริ่มตันตั้งแต่ ~12 เครื่องที่ poll 1 วินาที — **อย่านับ 64 เป็นสเปกที่ผ่านการทดสอบหรือ vendor รับรอง**
>
> ก่อนหน้านี้ (บันทึกจาก bring-up 2026-07-31) ย่อหน้านี้เขียนว่า "63 กล่อง" — ตัวเลขนั้น**ก็เป็นการอนุมานแบบเดียวกัน**จาก address space ที่ตอนนั้นเข้าใจผิดว่าเป็น 0–63 (ผู้ใช้ส่งตาราง SW1/SW2 จากคู่มือมาแก้เมื่อ 2026-08-05 ว่า address space จริงคือ 1–64) การแก้ 63→64 รอบนี้จึง **แก้แค่เลขนับ ไม่ได้ทำให้ตัวเลขกลายเป็น vendor-confirmed** — ยังเป็นการอนุมานเหมือนเดิม เพียงแต่คำนวณถูกขึ้น
