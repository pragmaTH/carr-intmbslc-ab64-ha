...
## 10. Write-to-readback latency — วัดจริงครั้งแรก (2026-08-05)

**ข้อมูลจากเครื่องเดียว** เหมือน section 9 — Carrier 38TGV0391A3 / 40TGV0391UP ยังไม่มีข้อมูลข้ามรุ่น **คู่มือ vendor (Intesis INMBSTOS001R000 r2.7 EN) ไม่เคยระบุตัวเลขนี้ไว้เลย** — ไม่มีสเปกว่า AB64 ใช้เวลาเท่าไรกว่า register จะสะท้อนค่าที่เพิ่งเขียนไป ตัวเลขทั้งหมดในหัวข้อนี้มาจากการวัดจริงเท่านั้น

วิธีวัด: สั่งงานผ่าน HA MCP (`ha_call_service`) แล้วจับเวลาตั้งแต่ส่ง service call จนค่าที่อ่านได้พลิกเป็นค่าใหม่ — ทดสอบที่ poll interval = 1 วินาที

| คำสั่ง | เวลาที่วัดได้ (round-trip เต็ม) |
|---|---|
| Setpoint 23 → 22 | 1.71s |
| Mode cool → fan_only | 1.76s |
| Mode fan_only → cool | 1.65s |

ตัวเลขคงที่ทั้งสามครั้ง (1.65–1.76s) — ในนั้นมีราว **0.36 วินาที** ที่เป็น round-trip overhead ของเครื่องมือวัดเอง (HA MCP) ไม่ใช่ความหน่วงของ AB64/แอร์ ดังนั้นความหน่วงฮาร์ดแวร์จริงประมาณ **~1.3–1.4 วินาที**

**สิ่งที่ตัวเลขนี้บอก**: AB64 ใช้เวลาประมาณ 1 วินาทีกว่า register จะสะท้อนค่าที่เพิ่งเขียนไป — ถ้า integration อ่านซ้ำทันทีหลัง write (ก่อนครบ ~1 วินาที) จะได้ค่าเก่ากลับมา ต้องรอ poll รอบถัดไปถึงจะเห็นค่าใหม่ ที่ default poll 30 วินาทีจึงรู้สึกช้ามาก ทั้งที่ฮาร์ดแวร์พร้อมตั้งแต่วินาทีที่ 1 — ดูการแก้ไข (post-write refresh ~2 วินาทีหลัง write) ใน `README.md` และ decision log ที่ `CLAUDE.md`
